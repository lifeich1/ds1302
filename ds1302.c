  ...
  while (testVal != -1) {
    ...
    for (addr = 0; addr < 31; ++addr)
      if ((got = ds1302ramRead(addr)) != testVal) {
        printf(
            "DS1302 RAM Failure: Address: %2d, Expected: 0x%02X, Got: 0x%02X\n",
            addr, testVal, got);
        ++errors;
      }
    testVal = ramTestValues[++i];
...
static int setLinuxClock(void) {
  ...
  sprintf(dateTime, "%02d%02d%02d%02d%02d%02d.%02d",
          ...
          bcdToD(clock[RTC_MINS], masks[RTC_MINS]), 20,
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0;; ++i) {
    ...
    ds1302clockRead(clock);
    printf(" %2d:%02d:%02d", bcdToD(clock[2], masks[2]),
           bcdToD(clock[1], masks[1]), bcdToD(clock[0], masks[0]));

    printf(" %2d/%02d/%04d", bcdToD(clock[3], masks[3]),
           bcdToD(clock[4], masks[4]), bcdToD(clock[6], masks[6]) + 2000);
